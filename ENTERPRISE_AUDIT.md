...
- **P2-6 — Secrets in repo-tracked docs / shared DB password.** `CLAUDE.md` previously embedded the live Postgres password (redacted; rotated 2026-06-07) and referenced `BACKEND_ADMIN_TOKEN` in copy-paste run commands. The frontend also holds full DB credentials via `DATABASE_URL` (direct `pg.Pool`). **Partially remediated 2026-06-07:** Postgres password and `BACKEND_ADMIN_TOKEN` rotated, hardcoded values removed from docs and ops scripts (now sourced from `.env`). Still TODO: move secrets to a manager (GCP Secret Manager) and rotate the `hermes_user` DB password.
